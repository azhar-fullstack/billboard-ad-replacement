...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Billboard detect + ad replace (image or simple video).")
    p.add_argument("mode", choices=["image", "video"], help="Run on one image or a video")
    p.add_argument(
        "--model",
    ...
    p.add_argument(
        "--ad-image",
        default=str(root / "assets" / "sample_ad.webp"),
        help="Ad image used for replacement",
    )
    p.add_argument(
        "--input",
        default=str(root / "assets" / "sample_billboard.webp"),
        help="Input image/video path",
    )
    p.add_argument(
        "--output",
        default=str(root / "demos" / "replaced.jpg"),
        help="Output path",
    )
    # video-only options (ignored in image mode)
    p.add_argument("--detect-width", type=int, default=1280)
    p.add_argument("--replace-all", action="store_true")
    p.add_argument("--ema-alpha", type=float, default=0.75)
    p.add_argument("--hold-frames", type=int, default=2)
    p.add_argument("--use-grabcut", action="store_true")
    p.add_argument("--detect-every", type=int, default=1)
    p.add_argument("--blend-mode", default="alpha", choices=["alpha", "clone"])
    return p.parse_args()
